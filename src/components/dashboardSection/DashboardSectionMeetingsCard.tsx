            ...
            {Array.isArray(sortedMeetings) && sortedMeetings.length > 0 ? (
                sortedMeetings.map((meeting: Meeting) => (
                    <div className={isDarkMode ? "dashboardSectionMeetingsCardContentsDark"
                        : "dashboardSectionMeetingsCardContents"} key={meeting.id}>
                        <h3
                            className="dashboardSectionMeetingsCardTitle"
                        ...
                        <p className="dashboardSectionMeetingsCardDescription"
                            onClick={() => handleMeetingDetailsClick(meeting.id)}
                        >
                            {meeting?.summary?.summary.length > 100 ? meeting?.summary?.summary.slice(0, 100) + '...' : meeting?.summary?.summary}
                        </p>
                        <div className='dashboardMeetingsOptions'>
                            ...
                            />
                            {openMeetingDeleteModal && chosenIndex && chosenIndex === meeting.id &&
                                <DashboardMeetingDeleteModal
                                    onMouseEnter={() => handleShowMeetingDeleteModal(meeting.id)}
                                    onMouseLeave={handleHideMeetingDeleteModal}
                                    data={{ id: meeting.id, name: `Meeting held on ${getDateAdded(meeting.title.split(' ').pop() || '')}` }}
                                />
                            }
                        </div>
                    </div>
